...
| **subclass of** | --- |
| **domain of** | [tcs:endpoint](#tcsendpoint), [tcs:port](#tcsport), [tcs:contentType](#tcscontenttype) |
...
| | |
|----------|----------|
| **Definition** | A [Channel](#channel) whose transport is HTTP. Marks the channel as expecting an `tcs:endpoint` (URL of the receiving side), a `tcs:port` (the port on which the receiver listens) and optionally a `tcs:contentType` (the serialization the receiver expects). Used by [BridgeTransportCompiler](#compiler) to match [EntryBoundaryComponent](#entryboundarycomponent)s with [ExitBoundaryComponent](#exitboundarycomponent)s that speak the same transport. |
...
| **subclass of** | --- |
| **domain of** | [tcs:instantiates](#tcsinstantiates), [tcs:runs](#tcsruns), [tcs:serviceName](#tcsservicename) |
...
| | |
|----------|----------|
| **Definition** | The media type the receiving side of a cross-container [Channel](#channel) expects, forwarded by the sending side as the outgoing request's `Content-Type`. Same population/consumption pattern as [tcs:endpoint](#tcsendpoint): written by the Entry boundary's config compiler, read by the paired Exit compiler. Optional — an Exit compiler that finds none leaves its processor's own default standing. It matters whenever the receiver selects its parser from the request header rather than sniffing the body, which `sw:rdf-ingest-service` does. |
| **domain** | [tcs:Channel](#channel) |
| **range** | xsd:string |
<br>
...
| | |
|----------|----------|
| **Definition** | A docker-compose service name that a [DockerContainer](#dockercontainer) contributes to the generated compose file. Multi-valued: one stanza is emitted per [PipelineComponent](#pipelinecomponent) owning a `tcs:DockerComposeConfig`, and a container may instantiate several of them. Projected by `ContainerServiceNameCompiler` in the finalize phase out of the YAML held in the config's `tcs:literal` — the name is otherwise invisible to anything reasoning in triples, SHACL above all. A derived view, not a second source of truth: the literal stays authoritative. Consumed by `tcs:BridgeEndpointReachableShape` to check that each cross-container [tcs:endpoint](#tcsendpoint) names a service the build will actually declare. |
| **domain** | [tcs:DockerContainer](#dockercontainer) |
| **range** | xsd:string |
<br>
