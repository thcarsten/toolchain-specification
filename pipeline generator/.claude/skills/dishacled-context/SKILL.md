...
- **`tcs:`** — the *toolchain* ontology namespace (this project). Core classes
  ...
  Bridge transport metadata sits on the `tcs:Channel`, written by the Entry
  boundary's config compiler and read by the paired Exit compiler:
  `tcs:endpoint` (receiver URL), `tcs:port`, `tcs:contentType` (optional;
  the serialization the receiver expects). `tcs:serviceName` on a
  `tcs:DockerContainer` is the compose service name(s) it contributes,
  projected out of the compose YAML by `ContainerServiceNameCompiler` so
  SHACL can reason over it.
  ...
  instances (typed by inference from either predicate, or by
  `PipelineEnricher`/`SemanticModelMapper` when minted at compile time).
  Authoring-layer alternative to writing `tcs:readsFrom`/`tcs:writesTo`
  by hand: `tcs:Connection` (`tcs:from`/`tcs:to`, exactly 1:1, no
  branching yet) — mapped into that internal wiring by
  `SemanticModelMapper` and never seen by anything downstream of it.
  Runtime flag: `tcs:isFinishing` (stripped before `compile()` returns).
...
- [x] **`dct:requires` closure re-expanded after bridge insertion**
  (2026-09-15). `PipelineAssembler` took the closure of the components
  *declared* in the pipeline and the runner runs each compiler once, so a
  component `BridgeTransportCompiler` inserted later brought no
  requirements with it: an auto-inserted `sw:rdf-ingest-service` bridge
  emitted a compose file with no `identifier` / `dispatcher` / `database`
  while its `tcs:endpoint` named the gateway. Fixed by extracting the
  minting pass to `utils.mint_missing_containers` (idempotent, skips
  microservices the build already carries a container for) and adding
  `RequirementClosureCompiler`, which re-runs it — gated on
  `BridgeTransportCompiler` having run, so it stays inert on pipelines
  that declare both boundary sides. `PipelineAssembler.describe_step`
  needs no second pass: the components added run no steps, and the
  bridge step was already placed by `BridgeTransportCompiler`.
  The general backstop is `tcs:BridgeEndpointReachableShape`: every
  cross-container `tcs:endpoint` hostname must match a compose service
  some container in the build declares, for all four frameworks. It lives
  in SHACL rather than a compiler so validation stays in one place, and
  reads `tcs:serviceName`, projected by `ContainerServiceNameCompiler` in
  the finalize phase (the compose service name is otherwise locked inside
  YAML in a `tcs:literal`, which SHACL cannot parse). Reported, not
  raised — generation still emits its files, as with
  `tcs:CatalogMissingBridgeShape`.
- [ ] **Auto-bridging breaks a channel named in a framework config**
  (2026-09-15). `BridgeTransportCompiler` repoints the original writer's
  `tcs:writesTo` at a fresh intra-container channel and gives the
  cross-container one to the inserted Exit step — but leaves the
  *framework-specific* config that also names the channel untouched. With
  `rdfc:Sdsify`, whose `rdfc:output` names its outputs explicitly, the
  emitted `pipeline.ttl` comes out disconnected and inverted:
  `:bridgestep_0 a rdfc:HttpOut ; rdfc:reader :channel_bridge_0 ;
  rdfc:writer demo_ab:sdsViolations` while `SdsifyViolations` still has
  `rdfc:output demo_ab:sdsViolations`. Reproducing case:
  `data/pipelines/pipeline_definition_autobridge.ttl`. Surfaced by the
  config-vs-annotation shape, so the validation report already flags it.
  The existing bridge tests miss it because their writer's output channel
  is synthesized from `p-plan:isPrecededBy` and never named in a config.
  **Deferred by the user (2026-09-15): to be addressed in a future
  iteration through a different mechanism — do not fix this in place.**
  The option considered and set aside was making `RdfcConfigCompiler`
  derive channel names from the live `tcs:writesTo`/`tcs:readsFrom`
  instead of the authored `rdfc:output`; recorded only so the same ground
  is not re-covered. Until the new mechanism lands, auto-bridging is
  usable where the writer does not name its channel in config (LDIO, NiFi)
  and not where it does (RDF-Connect). Still open alongside it: whether
  `rdfc:writer` belongs on an `rdfc:HttpOut` at all, since it is an Exit
  and only POSTs outward.
- [ ] **Hand-declared `tcs:DockerContainer`s are not honoured.**
  `PipelineAssembler.describe_docker_container` claimed in its docstring
  that a container declared in a pipeline definition is reused instead of
  minting a new `:container_N`; it never was, and the docstring is now
  corrected rather than the behaviour. The blocker is that such a
  container has no `dct:hasPart` edge from the build — `PipelineSeeder`
  mints the build node at compile time, so an author has nothing to
  attach it to. Making them first-class needs a way to adopt a declared
  container into the seeded build.
- [ ] **Should `tcs:DockerComposeConfig` bodies move from `tcs:literal` to
  `tcs:embedded`?** (2026-09-15). Raised while making the compose service
  name queryable. Evidence gathered then, for whoever picks this up:
  across all 12 service stanzas in the catalogs the only list-valued keys
  are `volumes` (8), `ports` (2) and `entrypoint` (1); `volumes` and
  `ports` are semantically *sets* in compose, so repeated predicates
  encode them losslessly, and only the NiFi `entrypoint`
  (`['/bin/bash','-c',<script>]`) is order-critical. No compose literal
  contains comments or block scalars, so YAML→RDF→YAML fidelity loss is
  near zero *today*. Ordering is expressible via JSON-LD `@list` →
  `rdf:List`, but `rdf:List` is painful in SPARQL/SHACL — an explicit
  index (`tcs:argument [ tcs:index 0 ; tcs:value … ]`) would be friendlier
  — and compose's full schema has ordered lists the catalog does not use
  yet. **For:** 29 call sites currently re-parse those YAML literals, so
  the literal is already being used as a queryable structure, badly;
  embedding would delete `parse_docker_compose_config`, make
  `tcs:serviceName` redundant, and enable real validation (port
  collisions, unpinned images, volume paths). **Against:** byte-identical
  reproduction of the demonstrator configs is a stated criterion
  (README §3) and RDF is unordered, so emitted YAML key order would need
  explicit canonicalisation — see `tests/test_docker_compose_determinism.py`.
  If pursued, embed by default with a `tcs:literal` override that wins
  when present, for stanzas whose YAML will not round-trip.
- [ ] **Dispatcher routes are hardcoded in `:MuDispatcherExDefault`**
  (2026-09-15). Adding `sw:rdf-ingest-service` meant hand-adding a
  `/ingest` route next to the existing `/sparql` one, in a config labelled
  `tcs:DefaultConfig`. Both routes ship regardless of whether the pipeline
  includes the service that serves them. Routes should be contributed by
  the components (`MuDispatcherCompiler` collecting them from the build)
  rather than baked into the default body — same override-mechanism story
...
- [x] **`SemanticModelMapper`** (the `SemanticModelVersionMapper` item below,
  now with its first concrete job) — translates the authoring-layer
  `tcs:Connection`/`tcs:from`/`tcs:to` vocabulary into internal
  `tcs:readsFrom`/`tcs:writesTo`/`tcs:Channel` wiring at the head of the
  ...
  - [ ] Branching (fan-out/fan-in) over `tcs:Connection` — needs a
    ...
    Until then, `tcs:readsFrom`/`tcs:writesTo` remain the supported
    escape hatch for branching pipelines.
  - [ ] Remove `tcs:ConnectionCardinalityShape` (marked `TEMPORARY` in
    `catalog-application-profile-shapes.ttl`) once branching lands.
  Decoupling the generator's internal model from a *versioned* public
  semantic model (the original framing of this item) is still open —
  this iteration only gave the mapper its first real translation job.
- [x] **`tcs:ConnectionCardinalityShape` message fix** (2026-09-15) — the
  two `sh:maxCount 1` property shapes now carry their own `sh:message`
  (the parent node shape's `sh:message` never propagated into nested
  property shapes in pySHACL), so violations report the intended
  "Branching is not yet supported" text instead of pySHACL's generic
  default.
- [x] **`rdfc_catalog_harvest.emitter` climb-arithmetic bug closed**
  (2026-09-15) — `owl_imports_path` over-climbed by one `../` because
