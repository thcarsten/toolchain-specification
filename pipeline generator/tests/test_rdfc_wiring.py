...
def test_reader_injected_when_unambiguous(catalog_graph):
    # rdfc:LogProcessorJs's placeholder configShape declares exactly one
    # sh:class rdfc:Reader path (rdfc:reader) and no writer path.
    parse_extra(
        catalog_graph,
        PREFIXES + """
...
def test_writer_injected_when_unambiguous(catalog_graph):
    # rdfc:HttpServer's placeholder configShape declares exactly one
    # sh:class rdfc:Writer path (rdfc:writer) and no reader path.
    parse_extra(
        catalog_graph,
        PREFIXES + """
        demo:Test a tcs:PipelineDefinition .
...
def test_ambiguous_writer_paths_left_unwired(catalog_graph):
    # rdfc:Sdsify's placeholder configShape declares two sh:class
    # rdfc:Writer paths (rdfc:output / rdfc:metadataOutput) — genuinely
