...
- ``Tier.BOOTSTRAP`` — seed the build graph from the catalog
...
``tier`` class attribute lets drivers sequence compilers (BOOTSTRAP
...
class Compiler(ABC):
    ...
    within :meth:`compile` to register their output as a ``tcs:File``
